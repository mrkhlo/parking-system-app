...
public class PaymentRefundEventKafkaPublisherIT {
    ...
    @Autowired
    private PaymentRefundEventKafkaPublisher paymentRefundEventKafkaPublisher;

    @Autowired
    private KafkaTemplate<String, PaymentRefundEventAvroModel> kafkaTemplate;

    @Autowired
    private ConsumerFactory<String, PaymentRefundEventAvroModel> consumerFactory;
    ...
    void should_PublishCorrectEvent() throws JsonProcessingException {
        //given
        String topicName = paymentServiceConfigData.getPaymentRefundEventTopicName();
        ...
        //when
        paymentRefundEventKafkaPublisher.publish(paymentEventOutboxMessage, biConsumer);

        //then
        Awaitility.await()
                .pollInterval(new FixedPollInterval(Duration.ofMillis(500)))
                .atMost(5, TimeUnit.SECONDS).until(() -> {
                    ConsumerRecord<String, PaymentRefundEventAvroModel> consumerRecord =
