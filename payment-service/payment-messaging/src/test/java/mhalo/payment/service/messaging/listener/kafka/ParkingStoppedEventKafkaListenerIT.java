...
public class ParkingStoppedEventKafkaListenerIT {

    @Autowired
    private KafkaProducer<String, ParkingStoppedEventAvroModel> kafkaProducer;

    @Autowired
    private PaymentServiceConfigData paymentServiceConfigData;

    @Autowired
    private ParkingStoppedEventListener parkingStoppedEventListener;
    ...
    void should_CallDomainListener() {
        ...
        var eventAvroModel = createRandomParkingStoppedEventAvroModel();
        String partitionKey = eventAvroModel.getParking().getParkingId().toString();

        //when
        kafkaProducer.send(paymentServiceConfigData.getParkingStoppedEventTopicName(),
                partitionKey, eventAvroModel, eventId);

        //then
        verify(parkingStoppedEventListener, after(2000).only()).process(any());
    }
    ...
    void should_RetryConfiguredTimes_When_RetryableExceptionThrown(Throwable exception) {
        ...
        String partitionKey = eventAvroModel.getParking().getParkingId().toString();
        doThrow(exception)
                .when(parkingStoppedEventListener).process(any());
        ...
        verify(parkingStoppedEventListener, after(timeout).times(kafkaConsumerConfigData.getRetryCount() + 1)).process(any());
    }
    ...
    private static Stream<Throwable> noopExceptionsMethodSource() {
        ...
        return Stream.of(
                violationException,
                new PaymentNotFoundException("something"),
    ...
    private ParkingStoppedEventAvroModel createRandomParkingStoppedEventAvroModel() {
        ParkingStoppedEventAvroModel event = new ParkingStoppedEventAvroModel();
